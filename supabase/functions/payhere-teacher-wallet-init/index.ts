...
Deno.serve(async (req) => {
  ...
  if (!authHeader?.startsWith('Bearer ')) {
    return json(req, { ok: false, error: 'not_authenticated' }, 401);
  }
  ...
  if (!Number.isFinite(amountCents) || amountCents <= 0) {
    return json(req, { ok: false, error: 'invalid_amount' }, 400);
  }
  ...
  if (!allowedReturn(returnUrl) || !allowedReturn(cancelUrl)) {
    return json(req, { ok: false, error: 'invalid_return_urls' }, 400);
  }
  ...
  if (!merchantId || !merchantSecret) {
    return json(req, { ok: false, error: 'payhere_not_configured' }, 503);
  }
  ...
  if (orderError || !orderData || typeof orderData !== 'object') {
    return json(req, { ok: false, error: orderError?.message ?? 'order_create_failed' }, 500);
  }

  const order = orderData as Record<string, unknown>;
  const checkoutToken = String(order.checkout_token ?? '');
  if (!checkoutToken) {
    return json(req, { ok: false, error: 'order_create_failed' }, 500);
  }
