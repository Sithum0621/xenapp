-- Per-class-slot attendance rows for calendar borders and day timeline bottom sheet.
...
REVOKE ALL ON FUNCTION public.student_attendance_group_occurrences(uuid, uuid, text, int, date, time) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.student_attendance_group_occurrences(uuid, uuid, text, int, date, time) TO authenticated;
