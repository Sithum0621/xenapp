-- Count attendance stats per class slot (weekly + extra), not per calendar day.
  ...
  FROM (
    ...
    LEFT JOIN LATERAL (
      SELECT
        ...
        COUNT(*) FILTER (WHERE occ.is_present)::int AS present_count,
        COUNT(*) FILTER (WHERE NOT occ.is_present)::int AS absent_count
      FROM public.student_attendance_group_occurrences(
        p_student_user_id,
        g.id,
        'institute',
        v_window,
...
BEGIN
  ...
  SELECT
    COUNT(*)::int,
    COUNT(*) FILTER (WHERE occ.is_present)::int
  INTO v_total, v_present
  FROM (
    SELECT occ.is_present
    FROM public.lecture_group_students lgs
    CROSS JOIN LATERAL public.student_attendance_group_occurrences(
      p_student_user_id,
