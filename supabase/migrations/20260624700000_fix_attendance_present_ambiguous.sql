-- Fix: RETURNS TABLE column "present" conflicted with SQL column "present" in RETURN QUERY
-- (PostgreSQL error: column reference "present" is ambiguous).
...
BEGIN
  ...
  all_days AS (
    SELECT class_date, day_present FROM occurrence_days
    UNION ALL
    SELECT e.class_date, e.day_present
  ...
  by_date AS (
    SELECT
      ad.class_date,
      BOOL_OR(ad.day_present) AS any_present,
